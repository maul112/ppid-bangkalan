    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-[2rem] border border-gray-100 shadow-sm">
            <div class="flex items-center gap-3 ml-2 md:ml-4 w-full md:w-auto">
                ...
                <span class="text-sm font-black text-gray-400 uppercase tracking-widest hidden sm:inline">Arsip Berita</span>
            </div>
            
            <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto mr-2">
                <form action="{{ route('admin.berita.index') }}" method="GET" class="w-full sm:w-auto flex gap-2">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul/kategori..." class="w-full sm:w-64 border border-gray-200 rounded-2xl text-sm font-bold text-gray-600 px-4 py-3 focus:ring-2 focus:ring-red-100 focus:border-red-300 transition-all">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.berita.index') }}" class="bg-gray-100 text-gray-600 px-6 py-3 rounded-2xl text-sm font-black transition flex items-center gap-2 hover:bg-gray-200">
                            RESET
                        </a>
                    @endif
                </form>

                <a href="{{ route('admin.berita.create') }}" class="w-full sm:w-auto justify-center bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-2xl text-sm font-black shadow-lg shadow-red-100 transition flex items-center gap-2">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gray-50/50 border-b border-gray-100">
                        <th class="px-8 py-5 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Gambar</th>
                        <th class="px-8 py-5 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Judul Berita</th>
                        <th class="px-8 py-5 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Tanggal Terbit</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/80 transition-all">
                        <td class="px-8 py-5">
                            @if($b->gambar)
                                <img src="{{ asset('uploads/berita/'.$b->gambar) }}" class="w-20 h-14 object-cover rounded-2xl shadow-sm ring-2 ring-gray-100">
                            @else
                                <div class="w-20 h-14 bg-gray-100 rounded-2xl flex items-center justify-center text-[10px] text-gray-400 font-bold uppercase">No Image</div>
                            @endif
                        </td>
                        <td class="px-8 py-5">
                            <span class="font-bold text-gray-800 text-lg block leading-tight">{{ $b->judul }}</span>
                            <span class="text-xs text-gray-400 mt-1 block">Oleh: Administrator</span>
                        </td>
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-2 text-gray-500">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                <span class="text-sm font-medium">{{ $b->created_at->format('d M Y') }}</span>
                            </div>
                        </td>
                        <td class="px-8 py-5">
